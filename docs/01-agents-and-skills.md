...
> 5 套 RAG 知识库是 AegIsLoop Adaptive 的"团队记忆"与"自适应"的语料底座;由 A6 QualitySteward 通过 S26 持续监测健康度,S27 触发补全;由 A0 Leader 统一调度检索。检索统一走 AgentTeams 编排下的 RAG 网关(Higress),召回 + 重排 + 注入上下文。
